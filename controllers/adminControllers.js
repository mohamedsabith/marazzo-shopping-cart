...
const adminLogin = (data) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (Validations.error) {
      return reject({
        status: false,
        adminLoginErr: Validations.error.details[0].message.replace(/"/g, ''),
      });
    }
...
const addCategory = (data) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (category) {
      return reject({
        status: false,
        categoryErr: 'Category name already exists. Try with another name?',
      });
    } else {
      ...
      await newCategory.save(async (err, res) => {
        if (err) {
          return reject({
            status: false,
            categoryErr: 'Something went wrong try again',
          });
        }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (category && subcategory) {
      return reject({
        status: false,
        subcategoryErr:
          'Already category and subcategory added. Try with another name?',
      });
    }
    ...
    await newSubCategory.save(async (err, res) => {
      if (err) {
        ...
        return reject({
          status: false,
          subcategoryErr: 'Something went wrong try again',
        });
      }
...
const addBrand = (data) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (Brand) {
      return reject({
        status: false,
        brandErr: 'Brand name already exists. Try with another name?',
      });
    }
    ...
    await newBrand.save(async (err, res) => {
      if (err) {
        ...
        return reject({
          status: false,
          brandErr: 'Something went wrong try again',
        });
      }
      return resolve({ status: 'ok', msg: 'Sub-category added Successfully.' });
    });
...
const addProduct = (data, proImg1, proImg2, proImg3, proImg4) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (Validations.error) {
      return reject({
        status: false,
        productAddErr: Validations.error.details[0].message.replace(/"/g, ''),
      });
    }
    ...
    if (disamt >= ogamt) {
      return reject({
        status: false,
        productAddErr:
          'Discount price must be less than or equal two actual price.',
      });
    }
    ...
    if (product) {
      return reject({
        status: false,
        productAddErr: 'Product name already exists. Try with another name?',
      });
    } else {
      var todayDateFormat = moment(new Date()).format('DD/MM/YYYY');
      ...
      await newProduct.save(async (err, res) => {
        if (err) {
          ...
          return reject({
            status: false,
            productAddErr: 'Something went wrong try again',
          });
        }
...
const editProduct = (id, data, proImg1, proImg2, proImg3, proImg4) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (Validations.error) {
      return reject({
        status: false,
        editProductErr: Validations.error.details[0].message.replace(/"/g, ''),
      });
    }
    ...
    await ProductModel.findByIdAndUpdate(
      ...
      .catch((err) => {
        ...
        reject({
          status: false,
          editProductErr: 'Something went wrong try again',
        });
      });
  });
};
...
const CreateCoupon = (data) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (Validations.error) {
      return reject({
        status: false,
        couponCodeErr: Validations.error.details[0].message.replace(/"/g, ''),
      });
    }
    ...
    if (coupon) {
      return reject({
        status: false,
        couponCodeErr: 'Coupon code already exists.',
      });
    } else {
      ...
      if (date == false) {
        return reject({
          status: false,
          couponCodeErr: 'Expire date must be greater than today date.',
        });
      }
      ...
      couponCodeDiscount.save(async (err, res) => {
        if (err) {
          return reject({
            status: false,
            couponCodeErr: 'Something went wrong try again',
          });
        }
        return resolve({
          status: true,
          msge: 'Coupon code created successfully.',
          data,
          res,
        });
      });
...
const orderEdit = (id, User, data) => {
  return new Promise(async (resolve, reject) => {
    await orderModel.updateOne(
      { user: User, product: { $elemMatch: { _id: id } } },
      { $set: { 'product.$.status': data.status } }
    );
    await orderModel.updateOne(
      { user: User, product: { $elemMatch: { _id: id } } },
      { $set: { 'product.$.paid': data.paid } }
    );
    resolve();
  });
};
