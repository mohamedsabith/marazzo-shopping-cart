...
router.get('/viewProduct/:id',async(req,res)=>{
   ...
   if(token){
    ...
    const totalAmt = await totalAmount(decode.email)
    const countCart = await cartCount(decode.email)
    res.render('user/productDetail',{product,countCart,user:decode.name,token:token,totalAmt})
   }
...
router.get('/cart',verifyUser,async(req,res)=>{
  ...
  res.render('user/cart',{cart,countCart,totalAmt,user:decode.name,token:token,empty})
...
router.get('/cart/:id',verifyUser,async(req,res)=>{
  ...
  userCart(req.params.id,user.email).then((result)=>{
...
router.post('/updateProfile/:id',upload.single("userimg"),(req,res)=>{
  ...
  const number=decode.phone_number
    userDetailsUpdate(req.params.id,number,userimage,req.body).then((result)=>{
      res.cookie("token", result.token, { httpOnly: true });
...
router.post('/coupon',(req,res)=>{
  const token = req.cookies.token;
  const decode= jwt.decode(token)
  checkCouponCode(req.body.coupon,decode.email).then((result)=>{
...
router.get('/wishlist',verifyUser,async(req,res)=>{
  ...
  const empty =wishlistProducts==null || wishlistProducts.product.length==0
  res.render('user/wishlist',{wishlistProducts,token:token,totalAmt,empty,countCart})
})
...
router.get('/billing',verifyUser,async(req,res)=>{
  ...
  res.render('user/billing',{userAddress:address.Useraddress,billingAddress:address.billingAddress,cart,countCart,totalAmt})
})
...
router.get('/category/:id',async(req,res)=>{
  ...
  if(token){
    ...
    const categories= await listAllCategory()
   res.render('user/categoryPage',{products:products.product,categories,subCategory:products.subCategory,token:token,totalAmt,countCart})
  }else{
    let products=await categoryProducts(req.params.id)
    const categories= await listAllCategory()
    res.render('user/categoryPage',{products:products.product,categories,subCategory:products.subCategory})
  }
})
...
router.post('/newAddress',(req,res)=>{
  const token = req.cookies.token;
  const decode= jwt.decode(token)
  billingAddress(req.body,decode.email).then((result)=>{
...
router.get('/payment',verifyUser,async(req,res)=>{
  ...
  res.render('user/payment',{cart,countCart,totalAmt,address,err:req.session.errorCoupon,netAmount,discountAmt:req.session.couponDiscount,coupon})
  req.session.errorCoupon=null
})
...
router.post('/verifyPayment',(req,res)=>{
  const token = req.cookies.token;
  const decode= jwt.decode(token)
  verifyPayment(req.body,decode.email).then(()=>{
