...
router.post(
  ...
  upload.fields([
    ...
    { name: 'thumb_img1', maxCount: 1 },
    { name: 'thumb_img2', maxCount: 1 },
    { name: 'thumb_img3', maxCount: 1 },
  ]),
  (req, res) => {
    ...
    addProduct(req.body, proImg1, proImg2, proImg3, proImg4)
      .then((result) => {
...
router.get('/addCategory', verifyAdminToken, async (req, res) => {
  ...
  res.render('admin/addCategory', {
    result,
    err: req.session.categoryErr,
    error: req.session.subcategoryErr,
  });
...
router.post(
  ...
  (req, res) => {
    let proImg1 = req.files.product_img
      ? req.files.product_img[0].filename
      : req.body.image1;
    let proImg2 = req.files.thumb_img1
      ? req.files.thumb_img1[0].filename
      : req.body.image2;
    let proImg3 = req.files.thumb_img2
      ? req.files.thumb_img2[0].filename
      : req.body.image3;
    let proImg4 = req.files.thumb_img3
      ? req.files.thumb_img3[0].filename
      : req.body.image4;
    editProduct(req.params.id, req.body, proImg1, proImg2, proImg3, proImg4)
      .then(() => {
        res.redirect('/admin/listProducts');
      })
      .catch((err) => {
