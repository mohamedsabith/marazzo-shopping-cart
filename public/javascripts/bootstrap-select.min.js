    ...
    (e.prototype = {
      ...
      createDropdown: function () {
        var b = this.multiple ? ' show-tick' : '',
          ...
          e = this.$element.parents().hasClass('form-group-lg')
            ? ' btn-lg'
          ...
          h = this.options.actionsBox
            ? '<div class="bs-actionsbox"><div class="btn-group btn-block"><button class="actions-btn bs-select-all btn btn-sm btn-default">' +
              ...
              '</button><button class="actions-btn bs-deselect-all btn btn-sm btn-default">' +
      ...
      createLi: function () {
        var b = this,
          ...
          g = function (d, e, f, g) {
            var h = c(
              a.trim(a('<div/>').html(d).text()).replace(/\s\s+/g, ' ')
            );
            return (
              '<a tabindex="0"' +
              ('undefined' != typeof e ? ' class="' + e + '"' : '') +
              ('undefined' != typeof f ? ' style="' + f + '"' : '') +
              ('undefined' != typeof g ? 'data-optgroup="' + g + '"' : '') +
        ...
        return (
          this.$element.find('option').each(function () {
            ...
            if (
              ('' !== l && m && (l = '<span>' + l + '</span>'),
              c.data('content') ||
                (j = l + '<span class="text">' + j + k + '</span>'),
              !b.options.hideDisabled || !m)
            )
                  ...
                  var o = c.parent().attr('label'),
                    ...
                    q = c.parent().data('icon')
                      ? '<span class="' +
                        ...
                        '"></span> '
                      : '';
                  (o = q + '<span class="text">' + o + p + '</span>'),
                    0 !== n && d.length > 0 && d.push(f('', null, 'divider')),
                    d.push(f(o, null, 'dropdown-header'));
                }
      ...
      render: function (b) {
        ...
        b !== !1 &&
          this.$element.find('option').each(function (b) {
            c.setDisabled(
              b,
              a(this).is(':disabled') || a(this).parent().is(':disabled')
            ),
          ...
          e = this.$element
            ...
            .map(function () {
              var b,
                d = a(this),
                e =
                  d.data('icon') && c.options.showIcon
                    ? '<i class="' +
                      c.options.iconBase +
                      ' ' +
                      d.data('icon') +
              ...
              return (
                (b =
                  c.options.showSubtext && d.attr('data-subtext') && !c.multiple
                    ? ' <small class="muted text-muted">' +
                      d.data('subtext') +
                      '</small>'
                    : ''),
                d.data('content') && c.options.showContent
                  ? d.data('content')
                  : 'undefined' != typeof d.attr('title')
                  ? d.attr('title')
                  : e + d.html() + b
              );
            })
        ...
        (this.options.title = this.$element.attr('title')),
          ...
          this.$button.attr(
            'title',
            a.trim(a('<div/>').html(f).text()).replace(/\s\s+/g, ' ')
          ),
        ...
        if (this.options.size !== !1) {
          var a = this.$menu
            ...
            d = this.options.header
              ? b.find('.popover-title').outerHeight()
              : 0,
            e = this.options.liveSearch
              ? b.find('.bs-searchbox').outerHeight()
              : 0,
            f = this.options.actionsBox
              ? b.find('.bs-actionsbox').outerHeight()
              : 0;
          a.remove(),
            this.$newElement
              .data('liHeight', c)
      ...
      setSize: function () {
        ...
        var b,
          ...
          n =
            ...
            parseInt(f.css('padding-bottom')) +
            parseInt(f.css('border-top-width')) +
            parseInt(f.css('border-bottom-width')),
        ...
        if (
          ...
          var s = function () {
            ...
            r(),
              (b = d - q),
              e.options.dropupAuto &&
                e.$newElement.toggleClass(
                  'dropup',
                  c > d && b - q < f.height()
                ),
              e.$newElement.hasClass('dropup') && (b = c - q),
              (a =
                h.length + h.filter('.dropdown-header').length > 3
                  ? 3 * i + q - 2
                  : 0),
              f.css({
                'max-height': b + 'px',
          ...
          (b = i * this.options.size + u * m + n),
            e.options.dropupAuto &&
              this.$newElement.toggleClass('dropup', c > d && b < f.height()),
            f.css({ 'max-height': b + j + k + l + 'px', overflow: 'hidden' }),
      ...
      setWidth: function () {
        if ('auto' == this.options.width) {
          ...
          a.remove(),
            this.$newElement.css(
              'width',
              Math.max(parseInt(b), parseInt(c)) + 'px'
            );
        } else
      ...
      selectPosition: function () {
        var b,
          ...
          g = function (a) {
            f
              .addClass(a.attr('class').replace(/form-control/gi, ''))
        ...
        this.$newElement.on('click', function () {
          ...
          a('html').on('click', function (b) {
            a(b.target).closest(d.$newElement).length < 1 &&
              f.removeClass('open');
          });
      },
      ...
      liveSearchListener: function () {
        ...
        this.$newElement.on('click.dropdown.data-api', function () {
          ...
          this.$searchbox.on('input propertychange', function () {
            b.$searchbox.val()
              ? (b.options.searchAccentInsensitive
                  ? b.$lis
                      .not('.is-hidden')
                      .removeClass('hide')
                      .find('a')
                      .not(':aicontains(' + c(b.$searchbox.val()) + ')')
                      .parent()
                      .addClass('hide')
                  : b.$lis
                      .not('.is-hidden')
                      .removeClass('hide')
                      .find('a')
                      .not(':icontains(' + b.$searchbox.val() + ')')
                      .parent()
                      .addClass('hide'),
                b.$menu.find('li').filter(':visible:not(.no-results)').length
                  ? d.parent().length && d.remove()
                  : (d.parent().length && d.remove(),
                    d
                      .html(
                        b.options.noneResultsText +
                          ' "' +
                          b.$searchbox.val() +
                          '"'
                      )
                      .show(),
                    b.$menu.find('li').last().after(d)))
              : (b.$lis.not('.is-hidden').removeClass('hide'),
                d.parent().length && d.remove()),
      ...
      keydown: function (b) {
        ...
        if (
          ((d = a(this)),
          (f = d.parent()),
          d.is('input') && (f = d.parent().parent()),
          (m = f.data('this')),
          m.options.liveSearch && (f = d.parent().parent()),
          m.options.container && (f = m.$menu),
          (e = a('[role=menu] li a', f)),
          (o = m.$menu.parent().hasClass('open')),
          !o &&
            /([0-9]|[A-z])/.test(String.fromCharCode(b.keyCode)) &&
            (m.options.container
              ? m.$newElement.trigger('click')
              : (m.setSize(), m.$menu.parent().addClass('open'), (o = !0)),
            m.$searchbox.focus()),
          m.options.liveSearch &&
            (/(^9$|27)/.test(b.keyCode.toString(10)) &&
              o &&
              0 === m.$menu.find('.active').length &&
              (b.preventDefault(),
            ...
            d.val() ||
              ...
              (0 === e.filter('.active').length &&
                (e = m.$newElement
                  .find('li')
                  .filter(
                    m.options.searchAccentInsensitive
                      ? ':aicontains(' + c(p[b.keyCode]) + ')'
                      : ':icontains(' + p[b.keyCode] + ')'
                  )))),
          e.length)
        ) {
          if (/(38|40)/.test(b.keyCode.toString(10)))
            (g = e.index(e.filter(':focus'))),
              (i = e.parent(':not(.disabled):visible').first().index()),
              (j = e.parent(':not(.disabled):visible').last().index()),
              (h = e
                .eq(g)
                .parent()
              ...
              m.options.liveSearch &&
                ...
                (i = e.filter(':not(.disabled):visible').first().data('index')),
                (j = e.filter(':not(.disabled):visible').last().data('index')),
                (h = e
                  .eq(g)
                  .nextAll(':not(.disabled):visible')
                  .eq(0)
              ...
              m.options.liveSearch
                ? (b.preventDefault(),
                  d.is('.dropdown-toggle') ||
                    (e.removeClass('active'),
                    e.eq(g).addClass('active').find('a').focus(),
                    d.focus()))
                : e.eq(g).focus();
          else if (!d.is('input')) {
            var q,
              ...
              s = [];
            e.each(function () {
              a(this).parent().is(':not(.disabled)') &&
                a.trim(a(this).text().toLowerCase()).substring(0, 1) ==
                  p[b.keyCode] &&
                s.push(a(this).parent().index());
            }),
              ...
              r != p[b.keyCode]
                ? ((q = 1), a(document).data('keycount', q))
                : q >= s.length &&
                  (a(document).data('keycount', 0), q > s.length && (q = 1)),
              e.eq(s[q - 1]).focus();
          }
          (/(13|32)/.test(b.keyCode.toString(10)) ||
            (/(^9$)/.test(b.keyCode.toString(10)) && m.options.selectOnTab)) &&
            o &&
            (/(32)/.test(b.keyCode.toString(10)) || b.preventDefault(),
            m.options.liveSearch
              ? /(32)/.test(b.keyCode.toString(10)) ||
                (m.$menu.find('.active a').click(), d.focus())
              : a(':focus').click(),
            a(document).data('keycount', 0)),
            ((/(^9$|27)/.test(b.keyCode.toString(10)) &&
              o &&
              (m.multiple || m.options.liveSearch)) ||
              (/(27)/.test(b.keyCode.toString(10)) && !o)) &&
