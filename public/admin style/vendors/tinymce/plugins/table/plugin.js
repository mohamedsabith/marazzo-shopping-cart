...
(function () {
    ...
    return descendant$1(container, firstSelectedSelector).bind(
      function (first) {
        return descendant$1(container, lastSelectedSelector).bind(
          function (last) {
            return sharedOne$1(lookupTable, [first, last]).map(
              function (table) {
                return {
                  first: first,
                  last: last,
  ...
  var resizableColumns = function (warehouse, isResizable) {
    ...
    range(warehouse.grid.columns, function (index) {
      var colElmOpt = Warehouse.getColumnAt(warehouse, index).map(
        function (col) {
          return col.element;
        }
      );
  ...
  var TableActions = function (editor, lazyWire, selections) {
    ...
    var execute = function (operation, guard, mutate, lazyWire) {
      return function (table, target) {
        ...
        return guard(table)
          ? operation(wire, table, target, generators, sizing).bind(
              function (result) {
    ...
    var setTableRowType = function (editor, args) {
      return extractType(args, ['header', 'body', 'footer']).each(
        function (type) {
          map(
  ...
  var registerCommands = function (
    ...
    var eraseTable = function () {
      return getSelectionStartCellOrCaption$1(editor).each(
  ...
  var caretPositionFromPoint = function (doc, x, y) {
    return Optional.from(doc.dom.caretPositionFromPoint(x, y)).bind(
      function (pos) {
